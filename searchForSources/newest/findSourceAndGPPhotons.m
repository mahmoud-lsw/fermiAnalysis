...
for i=1:rowsData 
    if (abs(data(i,5))<=boxSizeB && (abs(data(i,4))<=boxSizeL || abs(data(i,4))>=(360-boxSizeL)) || (abs(data(i,5))<=planeRemoval))
...
photonDeleteFile=['photonRowsToDelete_100_', catalogueName(1:end-4),'_' ,int2str(degreesAwayFromCenter),'deg_plane' , int2str(planeRemoval),'.txt'];
